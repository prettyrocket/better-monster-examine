 ...
 *
 * <p>Two values come from the enclosing Bucket row rather than the {@code drop_json} blob and are
 * assigned after parsing: {@link #pageNameSub} (the {@code page_name#version_anchor} variant key the
 * table is grouped by) and the {@link #rareDropTable} flag. The flag follows the same sentinel-string
 * convention as {@code MonsterData.default_version}: Bucket serialises it as {@code ""} when set and
 * omits it otherwise, so presence — not a boolean — is the signal (see {@link #isRareDropTable()}).
 */
@Getter
